...
void MainWindow::on_Login_btn_clicked() {
  ...
  if (valid) {
    ...
    QLabel * error = ui -> stackedWidget -> findChild < QLabel * > ("label_60");
    error -> hide();
    QString name = Etmp.getNom() + " " + Etmp.getPrenom();
    ...
    QString username = Etmp.getNom() +" "+ Etmp.getPrenom() +" "+ Etmp.getFonction();
    QString action = currentDateTimeString+"\n" + username +" : " +"Logged In";
    ButtonActionRecorder::recordButtonAction(action);
...
void MainWindow::on_pdf_aid_clicked() {
  ...
  QString username = Etmp.getNom() +" "+ Etmp.getPrenom() +" "+ Etmp.getFonction();
  QString action = currentDateTimeString+"\n" + username +" : " +"Exported Donations PDF";
  ButtonActionRecorder::recordButtonAction(action);
  qDebug() << "History updated";
}
...
void MainWindow::on_pdf_r_clicked() {
  ...
  QString username = Etmp.getNom() +" "+ Etmp.getPrenom() +" "+ Etmp.getFonction();
  QString action = currentDateTimeString+"\n" + username +" : " +"Exported Restaurant PDF";
  ButtonActionRecorder::recordButtonAction(action);
  qDebug() << "History updated";
}
...
void MainWindow::on_pushButton_304_clicked() {
  ...
  if (test1) {
      ...
      QString username = Etmp.getNom() +" "+ Etmp.getPrenom() +" "+ Etmp.getFonction();
      QString action = currentDateTimeString+"\n" + username +" : " +"Deleted a Benificiary";
      ButtonActionRecorder::recordButtonAction(action);
      qDebug() << "History updated";
...
void MainWindow::on_pushButton_364_clicked() {
  ...
  QString username = Etmp.getNom() +" "+ Etmp.getPrenom() +" "+ Etmp.getFonction();
  QString action = currentDateTimeString+"\n" + username +" : " +"Exported Benificiary PDF";
  ButtonActionRecorder::recordButtonAction(action);
  qDebug() << "History updated";
}
// benif search
...
void MainWindow::on_pushButton_30_clicked() {
  ...
  if (test4) {
      ...
      QString username = Etmp.getNom() +" "+ Etmp.getPrenom() +" "+ Etmp.getFonction();
      QString action = currentDateTimeString+"\n" + username +" : " +"Updated a Benificiary";
      ButtonActionRecorder::recordButtonAction(action);
      qDebug() << "History updated";
...
void MainWindow::on_chat_btn_clicked()
{
    MessengerServer server;
       ...
       QString currentDateTimeString = currentDateTime.toString("yyyy-MM-dd hh:mm:ss");
       QString username = Etmp.getNom() +" "+ Etmp.getPrenom() +" "+ Etmp.getFonction();
       QString action = currentDateTimeString+"\n" + username +" : " +"Started Using Messenger";
       ButtonActionRecorder::recordButtonAction(action);
       qDebug() << "History updated";
...
void MainWindow::on_history_button_clicked()
{
    ...
    QList<QString> actionsHistory;
    QListWidget *historyListWidget = ui->stackedWidget_2->findChild<QListWidget*>("listWidget");
    QFile file("Actions_history.txt");
        if (file.open(QIODevice::ReadOnly | QIODevice::Text))
        {
            QTextStream in(&file);
            while (!in.atEnd())
            {
                QString action = in.readLine();
                actionsHistory.append(action);

                // Create a QListWidgetItem and add it to the QListWidget
...
void MainWindow::on_btn_histor_to_dash_clicked()
{
    ui->stackedWidget_2->setCurrentIndex(1);
    QListWidget *historyListWidget = ui->stackedWidget_2->findChild<QListWidget*>("listWidget");
...
void MainWindow::on_pwd_generator_btn_clicked()
{
    QLineEdit *lengthLineEdit = ui->stackedWidget_2->findChild<QLineEdit*>("lineEdit_pwd_gen");


    int length = lengthLineEdit->text().toInt();
            if (length <= 0) {
                QMessageBox::warning(this, "Invalid Length", "Please enter a valid password length.");
                return;
            }

            QString password;
            const QString validCharacters = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890!@#$%^&*()-=_+";

            qsrand(QTime::currentTime().msec());

            for (int i = 0; i < length; ++i) {
                int index = qrand() % validCharacters.length();
                password.append(validCharacters.at(index));
            }
            QLineEdit *pwdgenerated = ui->stackedWidget_2->findChild<QLineEdit*>("pwd_generated_label");
            pwdgenerated->setText(password);

}

void MainWindow::on_back_pwd_clicked()
{
    ui->stackedWidget_2->setCurrentIndex(3);
}
...
void MainWindow::on_pwd_generator_btn_2_clicked()
{
    QLineEdit *lengthLineEdit = ui->stackedWidget->findChild<QLineEdit*>("lineEdit_pwd_gen");
    int length = lengthLineEdit->text().toInt();
            if (length <= 0) {
                QMessageBox::warning(this, "Invalid Length", "Please enter a valid password length.");
                return;
            ...
            QLineEdit *pwdgenerated = ui->stackedWidget->findChild<QLineEdit*>("pwd_generated_label");
            pwdgenerated->setText(password);
}

void MainWindow::on_btn_back_pwd_clicked()
{
    ui->stackedWidget->setCurrentIndex(3);
}
